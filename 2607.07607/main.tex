...
\subsection{Performance under $2$-qudit and $3$-qudit erasure errors}\label{subsec:supple_multi_erasure_noise}
...
\subsection{Performance under multi-qudit erasure noise}\label{subsec:supple_multiqudit_erasure}
...
The map $V_0:V_{\omega_1}\to V_{\omega_1}^{\otimes n}$ is an isometry and $SU(d)$-covariant, i. e.
\[
  V_0U(g)=U(g)^{\otimes n}V_0
  \qquad(g\in SU(d)).
\]
\end{smproposition}
...
Since $V_0$ is an $SU(d)$-intertwiner into the fundamental isotypic component, Schur's lemma gives a unit vector $\ket{m_0}\in M_{\omega_1}$ such that, after fixing the identification of the irrep factor,
\begin{equation}
\label{eq:seed-multiplicity-vector}
  V_0=W_{\omega_1}(I_{V_{\omega_1}}\otimes\ket{m_0}).
\end{equation}

For every unit vector $\ket m\in M_{\omega_1}$, define
\begin{equation}
\label{eq:Vm}
  V_m:=W_{\omega_1}(I_{V_{\omega_1}}\otimes\ket m):V_{\omega_1}\longrightarrow V_{\omega_1}^{\otimes n},
  \qquad
  \mathcal{E}_m(X):=V_mXV_m^\dagger.
\end{equation}
Every $V_m$ is an isometry and is exactly $SU(d)$-covariant.

\begin{smproposition}
\label{prop:orbit-twirl}
For every $X\in\End(V)$,
\begin{equation}
\label{eq:orbit-twirl}
  \mathbb{E}_{m\sim\mathrm{Haar}(M_{\omega_1})}\bigl[V_mXV_m^\dagger\bigr]
  =\frac1{n!}\sum_{\pi\in S_n}
  P_\pi\,V_0XV_0^\dagger P_\pi^\dagger.
\end{equation}
\end{smproposition}
...
\]
Conditioned on no collision, the $\ell$ sites come from distinct singlets.  Their state is exactly $\tau^{\otimes\ell}$ because different singlet blocks are independent and every one-site singlet marginal is $\tau$.  Thus
\[
  \beta_{\ell,N}
...
For a subset $S\subset[n]$ of size $k$, use increasing order to identify $V_{\omega_1}^{\otimes S}$ with $V_{\omega_1}^{\otimes k}$ and define \textit{the erased-subsystem channel}
\begin{equation}
\label{eq:local-channel}
  \Phi_{S,m}(X)
  :=\Tr_{S^c}(V_mXV_m^\dagger).
\end{equation}
...
Let
    ...
    \end{gathered}
    \end{equation} 
    where $t_a^{(i)}$ is a fundamental of $t_a$ on the $i$-th physical space $V_{\omega_1}$. \elo{Are the same assumptions needed for this proposition?}
Then, for every $X\in\End(V)$,
\begin{equation}
...
\label{eq:mean-fidelity}
  \mathcal{F}(\overline\Phi_{k,n},\Lambda_{k,n})
...
\begin{proof}
  ...
  +O_{d,k}(n^{-2})\,\|X\|_1.
    \end{equation}
    Both $\overline\Phi_{k,n}$ and $\Lambda_{k,n}$ are $SU(d)$-covariant under the fundamental input action and the collective output action.  Indeed, every $\beta_{\ell,N}$ is $SU(d)$-invariant.  Therefore \ref{??} applies,
    meaning that optimum is attained on maximally mixed state $I_d/d$, i. e.
    \[
        \mathcal{F}(\overline\Phi_{k,n},\Lambda_{k,n})=f(\sigma_{k,n},\eta_{k,n})=f(\eta_{k,n}+\Delta_{k,n},\eta_{k,n}).
    \]
    Let $R\cong V_{\omega_1}$ be a reference and let
    \[
    \ket{\Phi_d}_{LR}=\frac1{\sqrt d}\sum_{a=1}^d\ket a_L\ket a_R,
    \qquad
    \Phi_{LR}:=\ket{\Phi_d}\bra{\Phi_d}
    \]
    be purification of maximally mixed state.
    Define the traceless two-body operator
    \begin{equation}
    \label{eq:Gamma}
    \Gamma_{jR}:=\Phi_{jR}-\tau_j\otimes\tau_R.
    \end{equation}
    Set
    \begin{align}
    \sigma_{k,n}
    &:=(\overline\Phi_{k,n}\otimes\id_R)(\Phi_{LR}),\\
    \eta_{k,n}
    &:=(\Lambda_{k,n}\otimes\id_R)(\Phi_{LR})
...
\label{eq:test-operator}
  ...
  W_{\omega_1}^\dagger\bigl(\ket v\bra u_S\otimes I_{S^c}\bigr)W_{\omega_1}
...
\begin{equation}
    \frac{\operatorname{Tr} A_{S ; a b ; u v}}{D_q}=\langle u| \bar{\Phi}_{k, n}\left(E_{a b}\right)|v\rangle .
\end{equation}
\end{smlemma}
...
We have
\begin{align*}
\langle u| \Phi_{S, m}\left(E_{a b}\right)|v\rangle&=\langle u|\operatorname{Tr}_{S^c}\left[V_m E_{a b} V_m^{\dagger}\right]|v\rangle\\
&=\operatorname{Tr}\left[\left(|v\rangle\left\langle\left. u\right|_S \otimes I_{S^c}\right) W_{\omega_1}\left(E_{a b} \otimes|m\rangle\langle m|\right) W_{\omega_1}^{\dagger}\right]\right.\\
&=\operatorname{Tr}\left[W_{\omega_1}^{\dagger}\left(|v\rangle\left\langle\left. u\right|_S \otimes I_{S^c}\right) W_{\omega_1}\left(\ket{a}\bra{b} \otimes|m\rangle\langle m|\right)\right]\right.\\
&=(\langle b| \otimes I) W_{\omega_1}^{\dagger}\left(|v\rangle\left\langle\left. u\right|_S \otimes I_{S^c}\right) W_{\omega_1}(|a\rangle \otimes I)\right.\\
&=A_{S ; a b ; u v},
\end{align*}
...
\[
\mathbb{E}_m\langle u| \Phi_{S, m}\left(E_{a b}\right)|v\rangle=\langle u| \bar{\Phi}_{k, n}\left(E_{a b}\right)|v\rangle.
\]
\end{proof}
...
\begin{proof}
    Define
    \[
        Z:=\langle u|\left(\Phi_{S, m}-\bar{\Phi}_{k, n}\right)\left(E_{a b}\right)|v\rangle=\langle m| A_{S ; a b ; u v}|m\rangle-\frac{\operatorname{Tr} A_{S ; a b ; u v}}{D_q},
    \]
    ...
    \]
    where we took $t=D_q^{-1/4}$. Random variable $Z$ represents the difference between a matrix element of $\Phi_{S, m}\left(E_{a b}\right)$ and the corresponding matrix element of $\bar{\Phi}_{k, n}\left(E_{a b}\right)$.  The total number of such matrix elements is $M_{n,k}$.  By the union bound,
    \[
    \mathbb{P}(\text { some entry is bad }) \leq M_{n, k} \frac{D_q^{1 / 2}}{D_q+1}.
    \]
    For "good" events, we have
    \[
    \left.\left|\langle u|\left(\Phi_{S, m}-\bar{\Phi}_{k, n}\right)\left(E_{a b}\right)\right| v\right\rangle \mid \leq D_q^{-1 / 4},
    \]
    therefore by Lemma~\ref{lem:entry-to-diamond}, we obtain
    \[
    \max _{S \mid=k}\left\|\Phi_{S, m}-\bar{\Phi}_{k, n}\right\|_{\diamond} \leq r_k^{3 / 2} D_q^{-1 / 4},
    \]
    which concludes the proof.
\end{proof}
...
\begin{smtheorem}\label{thrm:randomized_multiplicity_vector}
    ...
    $$
    such that each $z_s(T)$ is computable in time poly $(n, d)$, and
    \begin{equation}\label{eq:randomized_multiplicity_vector_bound}
    \mathbb{P}_s\left[\max _{|S|=k}\left\|\Phi_{S, m_s}-\bar{\Phi}_{k, n}\right\|_{\diamond}>r_k^{3 / 2} D_q^{-1 / 4}\right] \leq \frac{M_{n, k}}{\sqrt{D_q}}=e^{-\Omega_d(n)}.
    \end{equation}
\end{smtheorem}
\begin{proof}
...
D_q=e^{\Theta(n)}
$
tableaux. Instead, we generate all these phases from a very short random seed. The key idea is that we don't need independence of all phases, instead it is enough to have $4$-wise independence of the phases, which can be achieved with a seed of length $O(n \log d)$.
...
\[
Z(s):=\langle u|\left(\Phi_{S, m_s}-\bar{\Phi}_{k, n}\right)\left(E_{a b}\right)|v\rangle=\langle m_s| A_{S ; a b ; u v}|m_s\rangle-\frac{\operatorname{Tr} A_{S ; a b ; u v}}{D_q},
\]
...
the output is good with probability $1-e^{-\Omega(n)}$;
