...
\title{
SFY: Covariant codes for approximate analog fault tolerance
...
\caption{
    ...
    (a) On the left, the logical operation is shown as the fundamental representation of $SU(d)$ acting on the logical space; the red arrow denotes the Lie-algebra element $iH$, where $H$ is a Hamiltonian. On the right, the corresponding physical operation is shown as a transversal product action on the physical subsystems. Here $\lambda_0$ denotes the chosen single-site physical representation of $SU(d)$, so each physical subsystem $(j)$ carries an identical copy $U_{\lambda_0}^{(j)}(g)$, where $g\in SU(d)$ and the red arrows denote the corresponding local Lie-algebra generators $\pi_{\lambda_0}^{(j)}(iH)$. Thus the logical symmetry generator is implemented as a sum of local physical generators. 
    (b) On the left, the logical code space $V_{\omega_1}\cong\mathbb C^d$ is mapped into the physical Hilbert space by the encoding $\mathcal{E}$. On the right, the physical Hilbert space is decomposed into isotypic components $V_\lambda\otimes M_\lambda$. The labels $\lambda_a$ and $\lambda_b$ denote arbitrary irreducible representations appearing in this decomposition, while $\omega_1$ denotes the fundamental representation. The highlighted component $V_{\omega_1}\otimes M_{\omega_1}$ contains all physical copies of the logical representation. The covariance condition makes encdoign isometry $V$, corresponding to $\mathcal{E}$, an intertwiner: intuitively, applying the symmetry before encoding is the same as encoding first and then applying the physical symmetry. Therefore, choosing the code space reduces to choosing a multiplicity vector $\ket m\in M_{\omega_1}$, which selects one embedded copy $V_{\omega_1}\otimes\ket m\subset V_{\omega_1}\otimes M_{\omega_1}$. 
    (c) On the left, the logical space $V_{\omega_1}\cong\mathbb C^d$ is shown together with logical states $\rho_L$ supported on it. In the center, the physical Hilbert space is shown with the chosen embedded copy $\mathbb C^d\otimes\ket m$ representing the logical code space. On the right, the complementary channel $\widehat{\mathcal N}$ maps the encoded state to the environment output. The dashed box displays the single-site reduced state $\rho^{(i)}(\rho_L)$ seen by the environment after a single-site erasure. In the complementary-channel formalism, robustness is analyzed by asking how much information about $\rho_L$ is revealed to the environment. For the covariant codes considered here, the logical-state-dependent part of each local reduced state is suppressed with the system size $n$, making the environment output close to a constant channel.
    (d) The dynamical Lie algebra $\text{DLA}[H(t)]$ in the presence of finite symmetries is shown as a subalgebra of an invariant Lie algebra of the form $\mathfrak{su}(d_1)\oplus\mathfrak{su}(d_2)\oplus\mathfrak{su}(d_3)\oplus\cdots$. Below, the corresponding block encoding is illustrated: each block $\mathfrak{su}(d_i)$ is encoded using an $SU(d_i)$-covariant code, so the invariant part of the dynamics acts transversally block by block. The red crosses indicate multi-site erasure errors, including inter-block correlated erasures, whose effect is analyzed using the reduced-state and complementary-channel formalism.
}
\label{fig:theme}
...
\textcolor{orange}{\textbf{NR:} You should add some form of signposting that says that: "this paper proceeds as follows: in section A, we discuss this. in section B, we discuss this. in section C, we discuss this, etc. I think you should move your disccussion section here and call it results overview or something. That section gives a good summary of results and also shows the order in which you communicated things in the main text}
\sfy{Nikita, I agree. I am just now trying to reorganize the intro somewhat -- but using the discussion is a good idea. }
...
%=============================
\sfy{Alternative introduction:\\
I tried to keep all the references but I am not sure. Please check!}
\color{teal}
\section{Introduction}\label{sec:main_intro}

Digital quantum computing has made substantial progress in recent years. Programmable quantum processors have reached regimes that are difficult to simulate classically, and there is growing experimental evidence for useful pre-fault-tolerant quantum computations in specific settings~\cite{Arute2019,Kim2023}. These demonstrations do not yet rely on full quantum error correction. At the same time, scalable digital quantum computation has a well-developed fault-tolerance paradigm: long computations are protected by encoding logical qubits into many physical qubits, repeatedly extracting error information, and implementing logical gates in ways compatible with the code. While the overheads remain large~\cite{Fowler2019, Gidney2021}, the conceptual framework for fault-tolerant digital algorithms is in place.

Analog quantum platforms offer a complementary route to quantum simulation and computation~\cite{Bloch2012,Browaeys2020,Daley2022,Schaefer2020}. Recent experiments have prepared strongly correlated states, including Fermi-Hubbard systems, in regimes that remain challenging for classical methods~\cite{Xu2025,Mazurenko2017,Shao2024,Chalopin2024}. More generally, analog devices implement dynamics through continuously applied Hamiltonians rather than through a sequence of discrete gates. This point of view also extends beyond simulation: universal analog computation can be formulated by asking whether the available Hamiltonians generate a sufficiently large dynamical Lie algebra (DLA)~\cite{Tavis1968,DAlessandro2021,Chen2020,Albertini2018,Albertini2020,Wang2016,Wang2012,Zanardi2004,AlbertiniDAlessandro2025,Hu2025}. Thus, analog platforms provide a promising route toward useful quantum dynamics. However, the corresponding framework for protecting native continuous-time dynamics is much less settled than in the digital circuit model.

Noise control is therefore central in both digital and analog settings. For digital quantum computers, there is now a mature theory of fault tolerance, together with rapid experimental progress at the level of logical memories and small logical processors. In particular, break-even performance has been demonstrated in several platforms, where increasing the number of physical qubits reduces logical error rates~\cite{Krinner2022,AcharyaGoogle2025,RyanAnderson2021,daSilvaPaetznick2024,Reichardt2024Tesseract,Bluvstein2024,Reichardt2024NeutralAtom}. At the level of full algorithms, a fault-tolerant digital approach is also conceptually available: one can compile the desired computation into a protected circuit, for example using surface-code error correction and magic-state distillation. For analog simulation, this route is possible but indirect. It replaces continuous Hamiltonian evolution by long sequences of synthesized gates, with Trotterization or phase-estimation costs in addition to substantial fault-tolerance overheads~\cite{Reiher2017ReactionMechanisms,Kivlichan2020Trotter}.

This motivates noise-control strategies that are more native to analog dynamics. Several approaches have been proposed to understand, mitigate, or suppress noise in analog quantum simulation. Some works identify regimes where analog quantum simulators can exhibit a quantum advantage while remaining stable to noise~\cite{Trivedi2024AnalogStability}, and others show that noisy analog simulations can still have accuracy guarantees and evidence for classical hardness in open-system settings~\cite{Kashyap2025AnalogOpenSimulation}. Other approaches aim to reduce errors directly, for example by Hamiltonian reshaping or rescaling~\cite{Guo2025HamiltonianReshaping}, by encoding the target dynamics into the ground space of a penalty Hamiltonian~\cite{Cao2024RobustAnalog}, or by continuous syndrome monitoring and feedback correction~\cite{Atalaya2021ContinuousQEC}. These ideas show that analog noise control need not simply reproduce digital fault tolerance. What is still missing is a general encoding principle that simultaneously suppresses local noise and preserves the continuous encoded dynamics that make analog platforms natural.

A fundamental obstruction is provided by the Eastin--Knill theorem~\cite{Eastin2009}. In an encoded analog device, the natural fault-tolerant way to implement a continuous logical operation is to realize its generator transversally, as a sum of local physical generators acting across the code block. Equivalently, one wants the encoding to respect a continuous symmetry: the physical action of the symmetry should implement the desired logical action on the code space. This is precisely the \textit{covariance} condition. However, exact finite-dimensional quantum error-correcting codes that correct local errors cannot support nontrivial continuous transversal symmetries, as stated in the Eastin--Knill theorem~\cite{Eastin2009}. Thus continuous analog dynamics and exact transversal quantum error correction are in tension.

There are several ways to relax the assumptions behind this obstruction. One can use infinite-dimensional codes, such as continuous-variable covariant codes, where exact continuous symmetries can be compatible with error correction~\cite{Hayden2021}. Alternatively, one can keep exact finite-dimensional codes but restrict the set of correctable errors~\cite{Gupta2024}. A third possibility is to allow a controlled, nonzero recovery error, leading to approximate quantum error correction (AQEC)~\cite{Beny2010}. In this work, we focus on the third approach, namely approximate quantum error correction in finite-dimensional covariant codes.

Within this framework, covariant codes can support continuous groups of transversal logical operations at the cost of a controlled approximation error~\cite{Faist2020}. A sequence of works has established quantitative trade-offs between covariance and correctability, including information-theoretic lower bounds for covariant codes under erasure noise~\cite{Faist2020,Zhou2021,Faist2023,Kong2022,Yang2022}. Near-optimal scaling is known from symmetric random constructions, and explicit examples have been developed for $U(1)$ covariance, W-state-type codes, and related questions in encoding complexity~\cite{Lin2025,Faist2020,Alexander2025,Yi2024}. These works establish the basic principles and limits of covariant AQEC. However, many constructions are randomized, rely on reference frames or ancilla systems~\cite{Kong2022,Yang2022}, or address specific code families. 

Covariant AQECCs are also relevant beyond fault-tolerant computation. They address the general problem of protecting quantum information that carries a physical symmetry charge. In holography, quantum error correction provides a natural language for bulk/boundary reconstruction in AdS/CFT, while constraints on exact global symmetries in quantum gravity make approximate symmetry-compatible codes especially natural~\cite{Almheiri2015,Pastawski2015,HarlowOoguri2019}. Covariant codes are also closely connected to quantum reference frames and quantum metrology: the same symmetry constraints that limit correctability can be formulated in terms of reference-frame accuracy or quantum Fisher information~\cite{Hayden2021,Woods2020,Kubica2021}. Thus explicit non-Abelian covariant AQECCs provide concrete models for studying the interplay between symmetry, locality, and approximate correctability in several areas of quantum information and physics.

Taken together, these developments suggest covariant AQEC as a possible route toward fault-tolerant analog quantum computation. The idea is not to digitize the dynamics into a long fault-tolerant circuit, but rather to choose an encoded subspace that supports the relevant continuous evolution while suppressing the information leaked by local noise. Approximate correctability is essential in this setting: by allowing a controlled recovery error, one can evade the finite-dimensional Eastin--Knill obstruction while retaining transversal implementations of continuous symmetry generators. At the same time, covariant AQEC is not only a tool for analog computation. Understanding its capabilities and limitations is also important for quantum information more broadly, because it clarifies how symmetry, locality, and error correction can coexist, and may inform the design of symmetry-compatible techniques for fault-tolerant quantum computation. Despite recent progress, explicit and flexible constructions remain limited. Many existing approaches focus on Abelian or otherwise restricted symmetries, on randomized constructions, or on single-site noise models. In many physical settings, however, one expects non-Abelian symmetries and errors that can affect several subsystems or have a more general local structure. This motivates the central question considered in this work: can one construct finite-dimensional covariant approximate quantum error-correcting codes for non-Abelian symmetries, such as $SU(d)$, with controllable performance under general multi-qudit noise models?

In this work, we give a positive answer to this question by constructing explicit finite-dimensional AQECCs compatible with non-Abelian continuous symmetries, proving near-optimal performance guarantees for multi-qudit noise models, and providing an explicit recovery map. We design $SU(d)$-covariant codes in which the logical system transforms as the fundamental representation and the physical system consists of many local $SU(d)$ degrees of freedom. The guiding idea is simple. Representation theory fixes where a covariant copy of the logical system can be embedded into the physical Hilbert space, but it leaves a controlled freedom in how this copy is chosen. We use this freedom to impose permutation symmetries on the code space, thereby spreading the logical information nearly uniformly across the physical subsystems. Since an erasure environment sees only reduced states on the erased subsystems, this local uniformity directly controls the information that leaks to the environment.

We make this mechanism quantitative by explicitly computing the relevant reduced states. For one-, two-, and three-qudit flagged erasure models, i. e., erasure errors on known locations, we obtain worst-case purified-distance scaling $\Theta(1/n)$, matching known lower bounds for covariant codes up to constant factors~\cite{Faist2020,Zhou2021}. For single-qudit erasure, we also give an explicit near-optimal decoder based on the Petz recovery map~\cite{Petz1988, Barnum2002, Ng2010}. The same reduced-state method applies beyond erasure noise: once the reduced state on the affected sites is controlled, arbitrary flagged local noise can be handled by applying the complementary channel of the local noise to this reduced state. This gives general, though not necessarily sharp, $O(n^{-1/2})$ correctability bounds for the single-qudit and three-qudit cases analyzed here.

We then use these codes as building blocks for robust analog quantum simulation. In many analog platforms, symmetries constrain the DLA generated by the available Hamiltonians. Rather than encoding the full algebra on the complete Hilbert space, one can exploit this structure: the symmetry-preserving dynamics is contained in an invariant Lie algebra with block form
$$
\mathfrak{su}(d_1)\oplus\mathfrak{su}(d_2)\oplus\cdots
\oplus \mathfrak{u}(1)^{\oplus(r-1)},
$$
where $r$ is some parameter defined later in the text. We therefore introduce a block-wise covariant encoding, using an $SU(d_i)$-covariant code for each non-Abelian block. This makes all dynamics generated inside the symmetry-preserving block algebra, and hence the original DLA contained in it, act transversally on the encoded space. We also analyze the resulting block code under both in-block and inter-block two-qudit erasures and show that the same $1/n$ scaling persists.

Finally, we discuss how symmetry-constrained analog dynamics can be extended toward universal analog computation. Symmetry-preserving Hamiltonians alone generally generate only the invariant block algebra, not the full algebra on the Hilbert space. We therefore first give a sufficient Lie-algebraic criterion ensuring that the invariant algebra, together with a suitable set of controlled symmetry-breaking Hamiltonians, generates the full algebra required for universality. This criterion is independent of any error-correcting encoding and applies directly at the level of universal analog controllability.

We then explain how this structure interacts with the block-encoding framework. The symmetry-preserving sector can be implemented transversally within the corresponding covariant code blocks. By contrast, the symmetry-breaking Hamiltonians connect different symmetry sectors and therefore are not transversal under the block encoding. In this sense, the protected symmetry-preserving dynamics form the native encoded operations, while the symmetry-breaking Hamiltonians act as non-transversal resource operations needed to recover universality.
...
$$
where $A_i$ corresponds to the Hilbert space of the $i$th qudit. For a subset $S\subseteq\{1,\ldots,n\}$, we write
$$
    A_S
    :=
...
%===========================
\sfy{A new version of section II (I left the old one above for now):}
\color{teal}
...
For the arbitrary flagged multi-qudit noise model~\eqref{eq:arbitrary_multiqudit_noise}, let $\widehat{\mathcal N}*S$ be a complementary channel to $\mathcal N_S$. Then, as shown in Lemma~C.11 of the Supplementary Material~\cite{SuppMat},
\begin{equation}\label{eq:dual_to_arbitrary_multiqudit}
\widehat{\mathcal N\circ\mathcal E}(\rho_L)
=
\sum_{S\in\mathcal S}
