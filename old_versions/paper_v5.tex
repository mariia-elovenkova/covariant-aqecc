...
The paper develops this story in three steps. We first set up the two ingredients used throughout the work (Section~\ref{sec:main_prelim}): approximate quantum error correction is formulated through the information leaked to the environment, and covariant encoding is formulated as the requirement that the physical symmetry action reproduce the logical one on the code space. We then build the codes themselves. The construction begins with the most transparent example, an $SU(2)$-covariant code that encodes a logical spin-$1/2$ into many spin-$j$ systems (Section~\ref{sec:main_su2}); this example already contains the central mechanism, namely that permutation symmetry makes the local reduced state almost independent of the logical input. The general $SU(d)$ construction follows next (Section~\ref{sec:main_sud}), where the same idea gives single-site erasure bounds, a Petz decoder, extensions to arbitrary flagged single-site noise, and higher-transitivity constructions for two- and three-site erasures. The second step turns these codes into a tool for analog simulation (Section~\ref{sec:main_covar_analog}): finite symmetries of the simulated system produce block-structured invariant Lie algebras, and the corresponding block encoding protects the symmetry-preserving dynamics while retaining good behavior under correlated erasures within and between blocks. The final step asks how this protected analog dynamics can become universal (Section~\ref{sec:main_univ_analog}). There, symmetry-breaking Hamiltonians are added as controlled non-transversal resources, completing the route from covariant AQEC to a framework for robust universal analog computation.

\section{Summary of Main Results}\label{sec:main_summary}
 
The following sections derive main results using a representation-theoretical framework. This section summarizes them with minimal use of that formalism, for readers interested in the formulas and bounds rather than the derivations. The subsections are presented in the same order as the corresponding sections of the paper.
 
\subsection{How we measure protection}\label{subsec:summary_framework}
...
The simplest version of the construction encodes a logical spin-$\tfrac12$ qubit into $n$ physical spin-$j$ systems ($n$ odd, $j$ half-integer), with logical $SU(2)$ rotations implemented as the total spin $J_a=\sum_k J_a^{(k)}$. Requiring the code to be invariant under cyclic permutations of the $n$ spins forces the logical information to be shared exactly equally among them. The reduced state of any single spin is then
$$
...
This bound is not just an existence statement: we provide an explicit recovery map, the Petz (transpose) map built directly from the encoding isometry $V$, and show that it achieves the same $1/n$ scaling, though we do not pin down its optimal constant.
 
The same reduced state also controls protection against noise that is not erasure. If a single qudit suffers an arbitrary unknown error rather than being erased, the observer can still only learn about $\rho_L$ through $\rho^{(i)}(\rho_L)$ above, so the code remains approximately correctable, with the weaker scaling $O(1/\sqrt n)$. The loss from $1/n$ to $1/\sqrt n$ comes from using a fully general, but less tight, proof technique (a trace-norm bound together with the Fuchs--van de Graaf inequality).
 
\subsection{Protecting against several erased qudits at once}\label{subsec:summary_multierasure}
 
If two or three qudits can be erased simultaneously, at unknown but flagged locations, cyclic symmetry is no longer enough: every pair, respectively every triple, of qudits must look alike to the environment. This requires the code to be invariant under a permutation group that can move any pair (or triple) of sites to any other pair (or triple) -- a $2$- or $3$-transitive subgroup of all permutations of the $n$ sites. Such subgroups are rare; we use the affine group $\mathrm{AGL}(1,n)$, Eq.~\eqref{eq:AGL_def}, for two-qudit erasures and the projective linear group $\mathrm{PGL}(2,n-1)$, Eq.~\eqref{eq:PGL_def}, for three-qudit erasures, both of which admit the needed invariant codes once $n$ (respectively $n-1$) is a suitable prime power. (The full permutation group $S_n$ would also work in principle, but it is too restrictive: no nonzero code is left invariant under all of $S_n$.)
 
With this extra symmetry, the three-qudit reduced state splits into a logical-independent piece and a logical-dependent piece of size $O(1/n)$, $\rho^{(ijk)}(\rho_L)=\tau^{(ijk)}+\Delta^{(ijk)}(\rho_L)$, Eq.~\eqref{eq:3q_reduced_state}, and the resulting bound for flagged three-qudit erasure is
$$
d(\mathcal R\mathcal N\mathcal E,\mathrm{id})\le \frac{\sqrt{3(d^2-1)}}{2\sqrt2}\,\frac1n+O(n^{-2}),
$$
again $1/n$ in $n$. As in the single-qudit case, the same reduced state controls protection against arbitrary (non-erasure) noise on three known qudits, at the weaker rate $O(1/\sqrt n)$.
